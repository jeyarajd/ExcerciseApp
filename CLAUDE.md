...
- Camera scoring: `Models/PoseScoring.swift` holds the pure logic (`ChairStandCounter`, `BalanceDetector`, `ReachEstimator`, tested with `BodyPose.sample`). `Features/Test/PoseCamera.swift` runs the front camera and Vision. Frames are never stored. `-demoScreen cameraChair|cameraBalance|cameraReach` plays scripted poses.
