...
- Demo launch arguments for screenshots: `-demoExercise <id> -demoTime <seconds>`, or (Debug builds) `-demoScreen <name>` (see `DemoScreen`, e.g. today, track, plan, test, sitRise, balance, chairStand, reach, result) with sample data, `-coachLook female|male`, `-voiceEnabled NO`, and `-demoSnapshot <file.png>` to save a picture and quit.
